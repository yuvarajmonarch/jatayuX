...
class GeminiService {
  ...
  async generateAnalysis(
    ...
    // If Gemini is unavailable → fallback
    if (!this.genAI) {
    ...
    try {
      ...
      // Parse JSON-structured AI result
  ...
  buildChatPrompt(context) {
    const { userMessage, previousAnalysis, spatialContext, messageCount } =
      context;
    ...
    const location = spatialContext?.coordinates
      ? `${spatialContext.coordinates.lat}, ${spatialContext.coordinates.lng}`
      : "Unknown";
...
🌡️ WEATHER CONDITIONS: ${
        ...
        weatherData.current_weather?.windspeed || "N/A"
      }km/h, Humidity: ${
        weatherData.hourly?.relativehumidity_2m?.[0] || "N/A"
      }%, Precipitation: ${
        weatherData.hourly?.precipitation?.[0] || 0
      }mm/h, Pressure: ${weatherData.hourly?.surface_pressure?.[0] || "N/A"}hPa
🌊 MARINE CONDITIONS: Wave Height: ${
        marineData.hourly?.wave_height?.[0] || "N/A"
      }m, Sea Temperature: ${
        marineData.hourly?.sea_temperature?.[0] || "N/A"
      }°C, Sea Surface Temperature: ${
        marineData.hourly?.sea_surface_temperature?.[0] || "N/A"
      }°C
🌬️ AIR QUALITY: AQI: ${airQualityData.data?.aqi || "N/A"}, Ozone: ${
        airQualityData.data?.ozone || "N/A"
      }μg/m³, PM2.5: ${airQualityData.data?.pm25 || "N/A"}μg/m³, PM10: ${
        airQualityData.data?.pm10 || "N/A"
      }μg/m³, NO2: ${airQualityData.data?.no2 || "N/A"}μg/m³
☀️ HEALTH METRICS: UV Index: ${
        healthData.data?.uvIndex?.index || "N/A"
      }, Pollen Level: ${pollenData.data?.level || "N/A"}, Pollen Count: ${
        pollenData.data?.count || "N/A"
      }, Allergen Risk: ${pollenData.data?.risk || "N/A"}
🚗 TRAFFIC & ACCESSIBILITY: Congestion: ${
        trafficData.data?.overallCongestion || 0
      }%, Average Speed: ${
        trafficData.data?.averageSpeed || "N/A"
      }km/h, Road Quality: ${
        trafficData.data?.roadQuality || "N/A"
      }, Accessibility Score: ${trafficData.data?.accessibilityScore || "N/A"}
🌱 ENVIRONMENTAL ANALYSIS: Land Use Change: ${
        changeDetectionData.data?.satelliteAnalysis?.landUseChange || "N/A"
      }%, Vegetation Change: ${
        changeDetectionData.data?.ndviVegetationAnalysis?.vegetationChange ||
        "N/A"
      }%, Urban Development: ${
        changeDetectionData.data?.satelliteAnalysis?.urbanChange || "N/A"
      }%, Land Type: ${
        landCoverData.data?.dominantType || "N/A"
      }, Vegetation Coverage: ${
        landCoverData.data?.vegetationCoverage || "N/A"
      }%, Change Intensity: ${
        changeDetectionData.data?.changeIntensity || "N/A"
      }`;
    } else {
      // Concise data for follow-up queries
      dataContext = `Location: ${location} | Temp: ${
        weatherData.current_weather?.temperature || "N/A"
      }°C | UV: ${healthData.data?.uvIndex?.index || "N/A"} | AQI: ${
        airQualityData.data?.aqi || "N/A"
      } | Wind: ${
        weatherData.current_weather?.windspeed || "N/A"
      }km/h | Wave: ${marineData.hourly?.wave_height?.[0] || "N/A"}m`;
    }

    // Generate contextual prompt based on user intent
    if (
      userMessageLower.includes("stay") ||
      userMessageLower.includes("safe") ||
      userMessageLower.includes("can i") ||
      userMessageLower.includes("dangerous")
    ) {
      if (isFirstQuery) {
...
RESPONSE FORMAT: Provide a comprehensive safety assessment with [YES/NO/CAUTION] verdict. Include detailed analysis of all relevant factors with specific data numbers. Explain risks, conditions, and recommendations in 3-4 sentences.`;
    ...
    if (
      userMessageLower.includes("water scarcity") ||
      userMessageLower.includes("water shortage") ||
      userMessageLower.includes("drought")
    ) {
      if (isFirstQuery) {
...
RESPONSE FORMAT: Provide a comprehensive water scarcity assessment with [HIGH/MEDIUM/LOW] risk level. Include detailed analysis of precipitation patterns, vegetation health, land use changes, and environmental factors. Give specific recommendations in 3-4 sentences.`;
    ...
    if (
      userMessageLower.includes("flood") ||
      userMessageLower.includes("flooding")
    ) {
      if (isFirstQuery) {
...
RESPONSE FORMAT: Provide a comprehensive flood risk assessment with [HIGH/MEDIUM/LOW] risk level. Include detailed analysis of precipitation patterns, land use changes, vegetation coverage, and terrain factors. Give specific recommendations in 3-4 sentences.`;
    ...
    if (
      userMessageLower.includes("development") ||
      userMessageLower.includes("change") ||
      userMessageLower.includes("growth") ||
      userMessageLower.includes("future") ||
      userMessageLower.includes("potential")
    ) {
      if (isFirstQuery) {
...
RESPONSE FORMAT: Provide a comprehensive development potential assessment with [HIGH/MEDIUM/LOW] potential rating. Include detailed analysis of growth trends, infrastructure capacity, environmental factors, and future projections. Give specific recommendations in 3-4 sentences.`;
    ...
    if (
      userMessageLower.includes("fishing") ||
      userMessageLower.includes("fish")
    ) {
      if (isFirstQuery) {
...
RESPONSE FORMAT: Provide a detailed spatial analysis covering all relevant factors. Use specific data numbers, explain conditions, assess risks/opportunities, and give actionable recommendations in 3-4 sentences.`;
  ...
  generateFallbackChatResponse(context) {
    ...
    const isWaterScarcityQuestion =
      userMessage.toLowerCase().includes("water scarcity") ||
    ...
    if (isWaterScarcityQuestion) {
      const coordinates = spatialContext?.coordinates || {
        lat: 18.5974354,
        lng: 73.710433,
      };
      ...
      return `Water Scarcity Analysis Complete for Lat: ${
        coordinates.lat
      }, Lon: ${coordinates.lng}
...
Based on the analysis, ${
        waterRisk === "high"
          ? "significant water scarcity concerns are evident in this area. The combination of low precipitation, poor vegetation health, and high urban development impact suggests urgent attention to water management is needed."
          : waterRisk === "medium"
          ? "moderate water scarcity risks are present. Monitoring precipitation patterns and vegetation health is recommended."
          : "water availability appears relatively stable, but continued monitoring is advised."
      }`;
    }
    ...
    if (
      ...
      return `Comprehensive Analysis Complete for Lat: ${
        coordinates.lat
      }, Lon: ${coordinates.lng}
...
Change Intensity: ${changeIntensity}`;
  ...
  buildAnalysisPrompt(
    ...
    return `
You are JatayuX — an AI Earth-observation assistant.
...

{
  ...
  "air_quality": {
    "aqi": "${airQualityData?.data?.aqi || "N/A"}",
    "pm25": "${airQualityData?.data?.pm25 || "N/A"}",
    "pm10": "${airQualityData?.data?.pm10 || "N/A"}",
    "ozone": "${airQualityData?.data?.ozone || "N/A"}"
  },
  ...
  "weather": {
    ...
    "precipitation": "${weatherData?.hourly?.precipitation?.[0] || "N/A"}"
  },
...
Fill in all fields *based on the input data and spatial reasoning*.
...
`;
  ...
  parseAIResponse(aiResponse) {
    ...
    let jsonString = extractJSON(aiResponse);
    ...
    try {
      ...
      let repaired = jsonString
        ...
        .replace(/[\u0000-\u001F]/g, ""); // remove invalid hidden chars
  ...
  generateFallbackAnalysis(
    ...
    const summary = `Comprehensive Analysis Complete for Lat: ${
      coordinates.lat
    }, Lon: ${coordinates.lng}
